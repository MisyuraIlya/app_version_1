...
export default class DocsHistoryAgent extends Component {
	constructor(props){
		super(props);
		this.state = {
			date: new Date(),
  ...
  constructUri = ()=>{
    ...
    if(searchParam.search('fromDate') == -1){
      let date = new Date();
      date = this.subtractMonths(date, 1);
      this.setState({date})
  ...
  updateUri = (searchWord, filterVal)=>{
    ...
    if(this.state.isChangePathToPageOne){
      pathname = '/DocsHistoryAgent/1/'+this.props.state.lang;
    }
  ...
  search = (e) =>{
	 ...
	 clearSearch = () =>{
		let pathname = '/DocsHistoryAgent/1/' + this.props.state.lang;
  ...
  goToItemFunc = (element) =>{
    ...
    this.props.history.push('/docsHistoryItems/'+ element.Id +'/'+this.props.state.lang);
  }

	render(){
    let pagination = [...Array(Math.ceil(this.state.items.length / this.state.toShow)).keys()];
    let breadCrumbsNav=[];
    let breadObject ={Title: 'הזמנות לקוחות',TitleEng: 'Profile', Link:"" + this.props.state.lang};
    ...
    let lang = this.props.state.lang;
        ...
        <BreadCrumbs breadCrumbsNav={breadCrumbsNav} title={''} searchQuery={''} lang={this.props.state.lang}/>
          ...
          />
          {/*
          <DocsTabs activeTab={0} lang={this.props.state.lang}/>
          */}
          <div className="for-calendar flex-container card">
                 ...
                 <tbody>
                    ...
                    {this.state.tempItems && this.state.tempItems.length ? this.state.tempItems.map((element, index) => {
                      ...
                      if( ( docAllowed == true) ){
                        ...
                        tmpDate = tmpDate[2]+"/"+tmpDate[1]+"/"+tmpDate[0];
                        let ordType = 'הזמנה';
                        if(element.OrdType == '3'){
                          ordType = 'החזרה';
                        }else if(element.OrdType == '2'){
                          ordType = 'ה.מחיר';
                        }
                        if(element.OrdStatus == 'Draft'){
                          ordType = 'טיוטת הזמנה'
                        }
                        let ordStatus = 'ממתין';
                        ...
                        return(
                            ...
                            <tr key={index} className={"item"} id={'docRow_' + element.Id} onClick={()=> this.goToItemFunc(element)}>
                                ...
                                <th className="col-cont sticky-col">
                                  <p className='AccountKey no-margin'>{element.ClientName}</p>
                                ...
                                <th className="col-cont">
                                  <p>{element.OrdExtId ? element.OrdExtId : '-'}</p>
                                </th>
                                <th className="col-cont">
                                  <p>{parseFloat(element.Total).toFixed(2)}</p>
                                </th>
            ...
            {this.state.tempItems && this.state.tempItems.length && this.props && this.state.paginateObj && Object.keys(this.state.paginateObj).length ?
									<ClientPagination paginateObj={this.state.paginateObj} headProps={this.props.match.params}
									            headLocation={this.props.location.search} lang={this.props.state.lang} urlNav='/DocsHistoryAgent/'/>
						: null}
