...
export default class docsAgent extends Component {
	constructor(props){
		super(props);
		this.state = {
			date: new Date(),
  ...
  updateUri = (searchWord, filterVal)=>{
    ...
    if(this.state.isChangePathToPageOne){
      pathname = '/docsAgent/1/'+this.props.state.lang;
    }
  ...
  search = (e) =>{
	 ...
	 clearSearch = () =>{
		let pathname = '/docsAgent/1/' + this.props.state.lang;
          ...
          <div className="for-calendar flex-container card">
                      ...
                      if( ( docAllowed == true) ){
                        ...
                        return(
                            ...
                            <tr key={index} className={"item"} id={'docRow_' + element.Id} onClick={()=> this.goToItemFunc(element)}>
                                <th className="col-cont sticky-col">
                                  <p className='AccountKey no-margin'>{'#' + element.DocNumber}</p>
                                </th>
                                <th className="col-cont sticky-col">
                                  <p className='AccountKey no-margin'>{'#' + element.AccountKey}</p>
                                  <p className='AccountName  no-margin'>{element.AccountName}</p>      
                                </th>
                                <th className="col-cont">
                                  <p>{docType}</p>
                                </th>
                                ...
                                <th className="col-cont">
                                  <p>{parseFloat(element.TFtal).toFixed(1)}</p>
                                </th>
                                <th className="col-cont col-approved">
                                  {element.Status==1?
                                    <p className='Active'>{element.ExtraText4 ? element.ExtraText4 : 'אושר'}</p>
                                    :
            ...
            {this.state.tempItems && this.state.tempItems.length && this.props && this.state.paginateObj && Object.keys(this.state.paginateObj).length ?
									<ClientPagination paginateObj={this.state.paginateObj} headProps={this.props.match.params}
									            headLocation={this.props.location.search} lang={this.props.state.lang} urlNav='/docsAgent/'/>
						: null}
