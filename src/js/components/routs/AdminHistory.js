  ...
  setPage(currentPage){
	...
	showInfo(element){
		...
		if (element.AddressJson) {
			html = `
			<div className="flex-container info-user">
    ...
    try {
      ...
      items.map(item => {
        orderCount++;
        ordSum += parseFloat(item.Total);
			});
      let infoObj = {
        orderCount: orderCount,
        ordSum: ordSum.toFixed(1)
      };

      this.setState({items, tempItems: items, search: false, preload:false, infoObj});
    } catch(err) {
        ...
        <div className="admin-history-sub">
          ...
          <div className="for-calendar flex-container">
            ...
            <div className="flex-container left-side-header col-lg-5">
              ...
              {Object.keys(this.state.infoObj).length ?
              ...
              */}
                  <div  className="left-side-comp userInfoCol col-lg-3">
                    <p>מס' הזמנות</p>
                    <p className="infoVal">{this.state.infoObj.orderCount}</p>
                  </div>
                  <div  className="left-side-comp userInfoCol col-lg-3">
                    <p>סה״כ הזמנות</p>
                    <p className="infoVal">{this.state.infoObj.ordSum}</p>
                  </div>
                </div>
              :null}
            </div>
                              ...
                              <div key={ind} className="product">
                                <div className="flex-container row-cls">
                                  ...
                                  <div className="col-lg-1 col-prod-quan">
                                    <div className="wrapp">
                                      <p>{el.Quantity ? parseFloat(el.Quantity).toFixed(1):null}</p>
                                    </div>
                                  </div>
