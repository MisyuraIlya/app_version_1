...
export default class MiniCart extends Component {
    ...
    debugger;
		return (priceArray.reduce(reducer)).toFixed(1);
	}
  												...
  												{this.props.state.openCart ?
  													<div className="details">
  														{/*{this.props.globalPriceCalc(element.Products, element.Quantity) == element.Products.Price * element.Quantity ?
  															<p className="price">{((parseFloat(element.Products.Price) * element.Quantity) * maam).toFixed(1)}</p>
  														:
  														<div className="price-widh-discount">
  															<p className="price">{(this.props.globalPriceCalc(element.Products, element.Quantity) * maam).toFixed(1)}</p>
  															<p className="old-price">{((parseFloat(element.Products.Price) * element.Quantity) * maam).toFixed(1)}</p>
  														</div>
                            }*/}
                              <p className="price">{(this.props.globalPriceCalc(element.Products, element.Quantity) * maam).toFixed(1)}</p>
  				...
  				<div className="footer">
  					<p className="to-pay">
  						...
  						{this.props.state.productsInCart.length && this.props.state.productSales.length ?
  							<span className="price">{(parseFloat(this.reducePrice()) * maam).toFixed(1)}</span>
  						:
