    ...
    paginationHandler(hydraView){
        ...
        if(hydraView["hydra:view"]) {
            ...
            if(hydraView["hydra:view"]["hydra:next"]) {
                ...
                if(hydraView["hydra:view"]["hydra:last"]) {
                    const next = (hydraView["hydra:view"]["hydra:last"]?.split('page='))[1]
                    nextPage = next
                } 
            }
    
            if(hydraView["hydra:view"]["hydra:previous"]) {
                const previous = (hydraView["hydra:view"]["hydra:previous"]?.split('page='))[1]
        ...
        return { totalPages, page, lastPage, nextPage, previousPage}
    },
}